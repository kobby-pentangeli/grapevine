...
- The crate's minimum supported Rust version is **1.88** (the let-chain syntax in the anti-entropy delivery path stabilized in 1.88); `rust-version` is set accordingly and a CI job enforces it.
- CI hardening: the test job runs `--all-targets --locked`; an MSRV check pinned to `rust-version` (1.88), a least-privilege top-level `permissions:` block, a `concurrency:` group that cancels superseded runs, and a single "CI success" gate job were added; the separate Windows build job folded back into the test matrix. The security workflow replaces its hand-rolled `cargo-audit` install with the maintained `rustsec/audit-check` action, and the commented-out `cargo-deny` stub was removed.
